# Pi0.5 VL cotrain smoke test plan

**Owner:** Sejohn · **Gate for:** Task #19 (full 24 h Brev launch) · **VM:** brev_instance2

Per the ObjectVLA spec + the enhanced
specification for the Pi0.5 VL cotrain (deprecated internal doc).
...
| Gate | Owner | Verify |
|---|---|---|
| Darius's Strix VRAM probe | Darius | Pi0.5 fits 16 GB, latency < 20 s on cold `pi05_base`. If RED → abort Pi0.5 VL cotrain entirely, pivot to SmolVLA. |
| Darius's VL pairs manifest | Darius | `HBOrtiz/so101_eval3_broad_grounding` on HF, parquet schema matches `lerobot_train_with_vl_cotrain.py` `VLPairsDataset.__init__` required cols (`image_path`, `prompt`, `target`) |
...
python eval_3/scripts/pi05_vl_cotrain/arcface_audit_200celeb.py \
    ...
    --output eval_3/scripts/pi05_vl_cotrain/audit_200celeb.parquet
```
...
python eval_3/scripts/pi05_vl_cotrain/build_keep_list_and_weights.py \
    --audit-parquet eval_3/scripts/pi05_vl_cotrain/audit_200celeb.parquet \
...
```bash
python eval_3/scripts/pi05_vl_cotrain/lerobot_train_with_vl_cotrain.py --help
```

**PASS criteria:** all Pi0.5 VL cotrain extra flags (`--vl_dataset.manifest`,
...
```bash
python -c "
import sys; sys.path.insert(0, 'eval_3/scripts/pi05_vl_cotrain')
...
s = build_curriculum_sampler(
    Path('eval_3/scripts/pi05_vl_cotrain/hardneg_weights.npy'),
    Path('eval_3/scripts/pi05_vl_cotrain/audit_200celeb.parquet'),
...
STEPS=200 BATCH_SIZE=8 bash eval_3/scripts/brev/train_pi05_vl_cotrain.sh
```
...
```bash
# On brev_instance2:
nohup bash eval_3/scripts/brev/train_pi05_vl_cotrain.sh \
    > ~/outputs/pi05_vl_cotrain.log 2>&1 &
echo $! > ~/outputs/pi05_vl_cotrain.pid
```
...
| Loss curve identical to vanilla Pi0.5's (no improvement) | confirm VL batches are actually firing (count `vqa_loss=` lines vs `flow_loss=` in log; should be ~1:10 ratio) |
