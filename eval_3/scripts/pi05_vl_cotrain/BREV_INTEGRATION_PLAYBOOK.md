# Pi0.5 VL cotrain - Brev integration playbook
...
cd ~/LeMonkey
git fetch origin
git checkout dev/SjohnU/pi05_vl_cotrain
git pull origin dev/SjohnU/pi05_vl_cotrain
```
...
import os
p = snapshot_download(
    repo_id="HBOrtiz/so101_eval3_broad_grounding",
    repo_type="dataset",
...
ds = VLPairsDataset(
    manifest_path_or_id="HBOrtiz/so101_eval3_broad_grounding",
...
This is the core of Pi0.5 VL cotrain. Pseudocode (adapt to your lerobot version):
...
wrapper already builds this dict from `layer_rank.json`. To apply it:

```python
from peft import LoraConfig, get_peft_model

# Build rank_pattern from layer_rank.json
rank_pattern = {
    rf".*language_model\.layers\.{i}\..*\.{tm}$": int(r)
    for i_str, r in json.load(open("eval_3/scripts/pi05_vl_cotrain/layer_rank.json"))["layer_rank"].items()
    for i in [int(i_str)]
...
STEPS=200 BATCH_SIZE=8 bash eval_3/scripts/brev/train_pi05_vl_cotrain.sh \
    > ~/outputs/pi05_vl_cotrain_smoke.log 2>&1 &
SMOKE_PID=$!
tail -f ~/outputs/pi05_vl_cotrain_smoke.log
```
...
Drop further:
```bash
STEPS=200 BATCH_SIZE=4 bash eval_3/scripts/brev/train_pi05_vl_cotrain.sh
```

If even bs=4 OOMs, the wrapper has a memory leak somewhere (likely the EMA
shadow). Disable EMA:
```bash
USE_EMA=false STEPS=200 BATCH_SIZE=8 bash eval_3/scripts/brev/train_pi05_vl_cotrain.sh
```
...
echo "PID=$(cat ~/outputs/pi05_vl_cotrain.pid); ETA ~24h"
```
...
| +24 h | Final checkpoint pushed to `HBOrtiz/so101_pi05_eval3` |
...
This is Task #20, separate from this playbook. Once `HBOrtiz/so101_pi05_eval3`
is pushed:
