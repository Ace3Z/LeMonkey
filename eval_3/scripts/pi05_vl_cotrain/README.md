...
| File | Purpose |
|---|---|
| `train_pi05_vl_cotrain_scaffold.py` | **Scaffold only; not the script that produced the deployed Pi0.5 checkpoint.** Mixed-batch Pi0.5 + VL cotrain wrapper around `lerobot-train`. Includes warm-PG start, audit/filter, hard-negative weights, per-layer LoRA rank (`layer_rank.json`), two-phase curriculum (`curriculum_sampler.py`), EMA. |
