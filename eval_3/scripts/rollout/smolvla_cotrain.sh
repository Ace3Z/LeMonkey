...
if [ -d "$REPO_ROOT/policy/so101_smolvla_eval3_cotrain" ]; then
  ...
  CACHE="$HOME/.cache/eval3_rollout/smolvla_cotrain_cam1"
fi
HOME_POSE=/tmp/run_rollout_smolvla_cotrain_home.json
HOME_DRIVE_S=2.0
...
mkdir -p "$ROLLOUT_DIR"
echo "Eval 3 - SmolVLA cotrain (in-distribution) - interactive rollout"
...
while true; do
  ...
  RUN_NAME="smolvla_cotrain_${i}_${TS}"
