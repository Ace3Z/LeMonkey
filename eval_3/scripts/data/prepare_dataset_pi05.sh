#!/usr/bin/env bash
# Prepare a Pi0.5-ready local copy of the augmented eval3 dataset.
#
# Materialises HBOrtiz/so101_eval3_cotrain locally (camera1 + meta
# + data only), swaps in the corrected stats.json from
# HBOrtiz/so101_eval3_cotrain_pi05, and applies the five schema renames
...
DATASET_ROOT="${DATASET_ROOT:-$HOME/datasets/eval3_cotrain_pi05}"
BASELINE_REPO="${BASELINE_REPO:-HBOrtiz/so101_eval3_cotrain}"
PI05_REPO="${PI05_REPO:-HBOrtiz/so101_eval3_cotrain_pi05}"
...
echo "    use --dataset.root=$DATASET_ROOT in train_pi05.sh"
