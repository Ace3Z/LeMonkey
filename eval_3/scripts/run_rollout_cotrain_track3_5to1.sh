#!/usr/bin/env bash
# Eval 3 rollout runner — HBOrtiz/smolvla_eval3_cotrain_track3_5to1_cam1.
#
...
# inference contract (`cam1`): supply only camera1; camera2/3 are auto
# zero-padded via the policy's empty_cameras=2 setting.
#
...
#   ./run_rollout_cotrain_track3_5to1.sh                 # default step_045000
#   ./run_rollout_cotrain_track3_5to1.sh step_020000     # an earlier checkpoint
#   ./run_rollout_cotrain_track3_5to1.sh /local/dir      # a custom local pretrained dir
set -euo pipefail

REPO_ID="HBOrtiz/smolvla_eval3_cotrain_track3_5to1_cam1"
ARG="${1:-step_045000}"
...
# HF's anonymous rate limit and stalls every rollout by ~8 min. The checkpoint
# is cached locally, so the probe adds nothing. (Safe to keep on any machine.)
...
mkdir -p "$ROLLOUT_DIR"
echo "Eval 3 — smolvla_cotrain_track3_5to1_cam1 — interactive rollout"
echo "Policy : $POLICY_PATH"
echo "Saving : $ROLLOUT_DIR"
echo "Schema : camera1 (live) + camera2/3 + empty_camera_0/1 auto-padded (empty_cameras=2)"
...
while true; do
  ...
  RUN_NAME="cotrain_track3_5to1_${i}_${TS}"
