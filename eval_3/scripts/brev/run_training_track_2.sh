...
#
# See eval_3/tracks/TRACK_OBJECTVLA.md (canonical) and
#     eval_3/tracks/TRACK_OBJECTVLA_ENHANCED.md (this enhanced spec).
#
# PRE-FLIGHT (must be true before running this):
#   1. Darius has pushed the VL pairs manifest HBOrtiz/eval3_objectvla_vl_pairs.
...
#   5. Smoke test passed (run with STEPS=200 first — see TRACK_OBJECTVLA.md §4).
...
SAMPLE_WEIGHTS="${SAMPLE_WEIGHTS:-eval_3/scripts/track_2/hardneg_weights.npy}"

# Enhancement B-4: per-layer LoRA rank config.
LAYER_RANK_CONFIG="${LAYER_RANK_CONFIG:-eval_3/scripts/track_2/layer_rank_track2.json}"
...
echo "==> Track 2 (Pi0.5 ObjectVLA enhanced) launching"
...
python -u eval_3/scripts/track_2/lerobot_train_with_vl_cotrain.py \
    --policy.type=pi05 \
...
echo
echo "==> Track 2 training done. Pushed to $PUSH_REPO."
