#!/usr/bin/env bash
# Brev-side env setup for the PaliGemma VQA warm-start (Track B fallback).
#
# Differs from brev_setup_track_B.sh in two ways:
...
echo "  3. Smoke test: bash eval_3/scripts/brev/run_warmstart_paligemma.sh after setting"
echo "     MANIFEST_PATH=... in env, with python eval_3/scripts/warmstart/train_paligemma_vqa.py --smoke"
echo "  4. Full run: bash eval_3/scripts/brev/run_warmstart_paligemma.sh"
