#!/usr/bin/env python3
"""Generate Track 2's bbox-grounded face VQA pairs from the 193-celeb scraped bank.

This is Darius's deliverable per `TRACK_OBJECTVLA.md` §2, but written so Sejohn
...
Per `TRACK_OBJECTVLA.md`: ObjectVLA's +45pp OOD lift depends on the BBOX
...
# Caption mix per canonical TRACK_OBJECTVLA.md §2.
...
def push_to_hf(parquet_path: Path, repo_id: str, token: str | None) -> None:
    ...
    api.upload_file(
        ...
        repo_id=repo_id,
        repo_type="dataset",
        commit_message="Track 2 VL pairs (bbox-grounded face VQA, 4 caption forms)",
    )
