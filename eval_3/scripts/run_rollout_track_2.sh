#!/usr/bin/env bash
# Interactive Track 2 rollout runner — Pi0.5 + ObjectVLA enhanced.
#
# Loads `HBOrtiz/pi05_eval3_objectvla` (Track 2 ObjectVLA bbox-grounded
# VQA co-train enhanced spec) from Hugging Face and feeds the typed
# prompt to the policy through lerobot-record.
#
# Adapted from eval_3/scripts/run_rollout.sh (Track C, SmolVLA, commit
...
#
# Usage:
#   ./run_rollout_track_2.sh                       # default revision: main
#   ./run_rollout_track_2.sh main                  # explicit main branch
#   ./run_rollout_track_2.sh <commit-sha>          # pin to a specific HF revision
#   ./run_rollout_track_2.sh /path/to/pretrained   # local pretrained_model dir
#
...
# Either prompt form should work — Track 2 trains on both (6 prompt patterns
# per eval_3/scripts/track_2/task_index_to_centroid.json).
#
...
REPO_ID="${REPO_ID:-HBOrtiz/pi05_eval3_objectvla}"
ARG="${1:-main}"
...
HOME_POSE="/tmp/run_rollout_eval3_track_2_home.json"
HOME_DRIVE_S=2.0
...
if [ -d "$ARG" ] && [ -f "$ARG/model.safetensors" ]; then
  ...
  POLICY_PATH="$CACHE_BASE/track_2_$(echo "$ARG" | tr / _)"
  mkdir -p "$CACHE_BASE"
...
mkdir -p "$ROLLOUT_DIR"
echo "Eval 3 (Track 2 — Pi0.5 ObjectVLA enhanced) — interactive rollout"
