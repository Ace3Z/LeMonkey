#!/usr/bin/env python3
"""Relabel Track 3 aug variant prompts before merging.
...
Usage:
    relabel_track3_prompts.py --aug-root datasets/eval3_track3_aug
    relabel_track3_prompts.py --dry-run                       # report only
    relabel_track3_prompts.py --limit 5                       # smoke
"""
...
def derive_target_name(sidecar_aug: dict) -> str:
    ...
    # Track 3 aug pipeline writes `new_target_short` AND `target_celeb_full`;
...
def main() -> int:
    ...
    print(f"found {len(var_dirs)} Track 3 aug variants under {args.aug_root}", flush=True)
    if not var_dirs:
        return 1
