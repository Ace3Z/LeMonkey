#!/usr/bin/env python3
"""Track 3 augmentation — tuple-driven, 3-celeb-only, full-enumeration capable.

Goal: train a SmolVLA 3-celeb safety baseline (Track 3) with maximum coverage
...
4 + 4 for obama). Built by build_track3_bank.py.
...
=====
    ...
    python generate_aug_track3.py \
        --base-root /home/rohamzn/ETH_Uni/LeMonkey/datasets/eval3 \
        --bank-root /home/rohamzn/ETH_Uni/LeMonkey/datasets/eval3_celebs/track3_bank \
        --out-root  /home/rohamzn/ETH_Uni/LeMonkey/datasets/eval3_track3_aug

    # Smoke-test: 3 tuples × 2 variants each = 6 variants
    python generate_aug_track3.py \
        --base-root ... --bank-root ... --out-root /tmp/track3_smoke \
...
def load_track3_bank(bank_root: Path) -> dict[str, list[Path]]:
    ...
    *,
    ...
    try:
        for v in variants:
            ...
            # 5. Prompt: 75/15/10 mix — reuses generate_aug_v3 helper.
            ...
            new_sidecar["source"] = "augmented_track3"
...
def main() -> int:
    ...
    # 1. Load bank
    bank = load_track3_bank(args.bank_root)
