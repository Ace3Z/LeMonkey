...
Visual-verification tooling for the co-training augmentation pipeline datasets. These scripts
...
| `verify_vl_pairs.py` | Renders panels from a VL-pairs manifest, frame-0 image with `quad_corners_norm` + `celeb_name` overlaid, plus the reference photo. Confirms label, bbox, and face all agree. |
